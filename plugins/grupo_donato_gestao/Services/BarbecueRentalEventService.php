...
final class BarbecueRentalEventService extends CustomerDataService
{
    private $events;
    ...
    public function __construct(int $unit_id, int $actor_id = 0, ?object $login_user = null)
    {
        ...
        $this->events = model("grupo_donato_gestao\\Models\\Gd_barbecue_rental_events_model");
    }

    public function append(int $rental_id, string $event_type, ?string $from_status = null, ?string $to_status = null, ?string $reason = null, array $payload = []): int
    {
        if (!in_array($event_type, Constants::COURT_RENTAL_EVENT_TYPES, true)) { throw new \DomainException("gd_invalid_court_rental_event"); }
