...
$buttons[] = anchor(get_uri("grupo_donato/court-rentals/monthly"), '<i data-feather="repeat" class="icon-16"></i> ' . app_lang("gd_menu_rental_monthly"), ["class" => "btn btn-default"]);
if (!empty($can_manage)) {
    $buttons[] = modal_anchor(get_uri("grupo_donato/court-rentals/single-modal"), '<i data-feather="plus-circle" class="icon-16"></i> ' . app_lang("gd_new_court_rental_single"), ["class" => "btn btn-primary", "title" => app_lang("gd_new_court_rental_single")]);
}
?>
...
<div id="page-content" class="page-wrapper clearfix gd-rentals-shell">
    <div class="card">
        <div class="page-title clearfix gd-page-header">
            <div>
                <h4><?php echo app_lang("gd_menu_rental_bookings"); ?></h4>
            </div>
            <?php if ($buttons) { ?><div class="title-button-group gd-toolbar"><?php echo implode(" ", $buttons); ?></div><?php } ?>
        </div>

        <div class="table-responsive">
            <table id="gd-court-rentals-table" class="display" cellspacing="0" width="100%"></table>
        </div>
    </div>
</div>
...
$(document).ready(function(){
    $("#gd-court-rentals-table").appTable({
        source:'<?php echo_uri("grupo_donato/court-rentals/single-data"); ?>',
        serverSide:true,
        order:[[2,"desc"]],
        filterDropdown:[
