...
class Barbecue_finance extends Gd_Controller
{
    private int $unit_id;
    private $db;
    ...
    public function create_rental_charge()
    {
        try {
            ...
            if (!$rentalRecord) {
                $this->json_error("Aluguel de churrasqueira não encontrado.");
                return;
            }
            ...
            $lockName = "grupo_donato_churrasqueira_mensalista_" . $this->unit_id . "_" . $rentalId . "_" . $reference;
    ...
    {
        $result = (new ReceivableGenerationService($this->unit_id, $this->user_id(), $this->login_user))->ensureMonth($reference, 'barbecue_rental');
        if (!empty($result['errors'])) {
            log_message('critical', 'Barbecue monthly receivable sync has errors: ' . json_encode($result['errors'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        }
    }
    ...
    private function baseQuery(array $options)
    {
        ...
        $rentals = $this->db->prefixTable("gd_barbecue_rentals");
        ...
        $query = $this->db->table($rentals . " cr")
            ...
            ->join($receivables . " r", "r.source_type='barbecue_rental' AND r.source_id=cr.id AND r.unit_id=cr.unit_id AND ((r.reference_month=$referenceSql) OR (r.reference_month='' AND cr.rental_type='single')) AND r.deleted=0", "left", false)
    ...
    private function row(object $data, int $month, int $year): array
    {
        ...
        return [
            anchor(get_uri("grupo_donato/barbecue-rentals/view/" . $data->rental_id), $this->escape($data->rental_number ?: "-")),
    ...
    {
        return $this->db->table($this->db->prefixTable("gd_barbecue_rentals"))
    ...
    private function createSingleRentalCharge(int $rentalId, object $rental): void
    {
        if ($this->existingReceivable($rentalId, "")) {
            $this->json_error("Este aluguel de churrasqueira já possui cobrança.");
            return;
        }

        $lockName = "grupo_donato_churrasqueira_avulsa_" . $this->unit_id . "_" . $rentalId;
        $lock = $this->db->query("SELECT GET_LOCK(" . $this->db->escape($lockName) . ", 10) AS lock_status")->getRow();
        if ((int) ($lock->lock_status ?? 0) !== 1) {
            $this->json_error("Não foi possível bloquear a criação da cobrança. Tente novamente.");
            return;
        ...
        try {
            ...
            $saved = $this->finance->createReceivable([
                ...
                "customer_account_id" => (int) $rental->customer_account_id,
                "reference_month" => "",
                "description" => "Aluguel de churrasqueira — " . $rental->title,
                "issue_date" => min(gmdate("Y-m-d"), $dueDate),
                "due_date" => $dueDate,
                "original_amount" => $rental->negotiated_amount,
                "unit_amount" => $rental->negotiated_amount,
            ...
            $this->json_success("Cobrança do aluguel de churrasqueira criada.", ["receivable_id" => (int) $saved["id"]]);
        } finally {
            $this->db->query("SELECT RELEASE_LOCK(" . $this->db->escape($lockName) . ")");
        }
    }
    ...
    private function monthlyRentalForReference(int $rentalId, string $reference): ?object
    {
        $first = $reference . "-01";
        $last = (new \DateTimeImmutable($first))->format("Y-m-t");
        return $this->db->table($this->db->prefixTable("gd_barbecue_rentals") . " cr")
    ...
    private function resourceFilterSql(int $resourceId): string
    {
        ...
        $type = $this->db->escape(Constants::BARBECUE_RESOURCE_TYPE);
        return "EXISTS (SELECT 1 FROM `$resources` rf WHERE rf.id=" . (int) $resourceId . " AND rf.unit_id=cr.unit_id AND rf.resource_type=$type AND rf.deleted=0 AND rf.is_active=1 AND rf.is_bookable=1) AND EXISTS (SELECT 1 FROM `$links` lf WHERE lf.rental_id=cr.id AND lf.unit_id=cr.unit_id AND lf.deleted=0 AND lf.link_kind <> 'historical' AND ("
    ...
    private function resourceNamesSql(): string
    {
        ...
        $resources = $this->db->prefixTable("gd_resources");
        $type = $this->db->escape(Constants::BARBECUE_RESOURCE_TYPE);
        $booking = "(SELECT GROUP_CONCAT(DISTINCT CONCAT(rb.code, ' - ', rb.name) ORDER BY rb.sort_order,rb.name SEPARATOR ', ') FROM `$links` lb JOIN `$bookingResources` br ON br.booking_id=lb.booking_id AND br.unit_id=lb.unit_id AND br.deleted=0 JOIN `$resources` rb ON rb.id=br.resource_id AND rb.unit_id=br.unit_id AND rb.resource_type=$type AND rb.deleted=0 WHERE lb.rental_id=cr.id AND lb.unit_id=cr.unit_id AND lb.deleted=0 AND lb.link_kind <> 'historical')";
        $series = "(SELECT GROUP_CONCAT(DISTINCT CONCAT(rs.code, ' - ', rs.name) ORDER BY rs.sort_order,rs.name SEPARATOR ', ') FROM `$links` ls JOIN `$seriesResources` sr ON sr.series_id=ls.booking_series_id AND sr.unit_id=ls.unit_id AND sr.deleted=0 JOIN `$resources` rs ON rs.id=sr.resource_id AND rs.unit_id=sr.unit_id AND rs.resource_type=$type AND rs.deleted=0 WHERE ls.rental_id=cr.id AND ls.unit_id=cr.unit_id AND ls.deleted=0 AND ls.link_kind <> 'historical')";
