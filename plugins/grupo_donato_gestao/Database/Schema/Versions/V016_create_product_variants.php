    ...
    public function up(BaseConnection $db, string $prefix): void
    {
        ...
        $this->ensureTable($db, $table, "
            CREATE TABLE IF NOT EXISTS `$table` (
                ...
                `active_code_key` VARCHAR(120) AS (IF(`deleted`=0, CONCAT(`product_id`, ':', `code`), NULL)) PERSISTENT,
                `default_variant_key` BIGINT UNSIGNED AS (IF(`deleted`=0 AND `status`='active' AND `is_default`=1, `product_id`, NULL)) PERSISTENT,
