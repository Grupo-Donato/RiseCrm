(function (window, document) {
    ...
    function saveView() {
        ...
        window.ImpulsoDialogs.prompt({ title: 'Salvar visualização', label: 'Nome da visualização', placeholder: 'Ex.: Minhas urgentes' }).then(function (name) {
            if (name == null || !name.trim()) return;
            bridge.api(bridge.endpoint('savedViews'), { method: 'POST', body: { name: name.trim(), schema_version: 1, filters: bridge.currentSavedViewFilters() } }).then(function (payload) { views.push(data(payload, {})); render(); }).catch(function (error) { if (bridge.toast) bridge.toast('Visualização não salva', error.message, 'alert-triangle'); });
        });
    }
    ...
    function renameView(id) { var view = views.find(function (item) { return Number(item.id) === id; }); if (!view || !window.ImpulsoDialogs) return; window.ImpulsoDialogs.prompt({ title: 'Renomear visualização', label: 'Nome da visualização', value: view.name }).then(function (name) { if (name == null || !name.trim()) return; bridge.api(bridge.endpointWithId('savedViews', id, ''), { method: 'PUT', body: { name: name.trim(), schema_version: 1, filters: view.filters } }).then(function (payload) { var next = data(payload, view); views = views.map(function (item) { return Number(item.id) === id ? next : item; }); render(); }).catch(function (error) { if (bridge.toast) bridge.toast('Visualização não renomeada', error.message, 'alert-triangle'); }); }); }
    function deleteView(id) { if (!window.ImpulsoDialogs || typeof window.ImpulsoDialogs.confirm !== 'function') return; window.ImpulsoDialogs.confirm({ title: 'Excluir visualização?', message: 'Esta visualização será removida para este usuário.' }).then(function (confirmed) { if (!confirmed) return; bridge.api(bridge.endpointWithId('savedViews', id, ''), { method: 'DELETE', body: {} }).then(function () { views = views.filter(function (item) { return Number(item.id) !== id; }); render(); }).catch(function (error) { if (bridge.toast) bridge.toast('Visualização não excluída', error.message, 'alert-triangle'); }); }); }
    ensurePanel(); load();
