...
use grupo_donato_gestao\Services\BarbecueRentalLifecycleService;
use grupo_donato_gestao\Services\BarbecueRentalService;
...
class Barbecue_rentals extends Gd_Controller
{
    private int $unit_id;
    private BarbecueRentalService $service;
    ...
    public function __construct()
    {
        ...
        $this->service = new BarbecueRentalService($this->unit_id, $this->user_id(), $this->login_user);
    ...
    {
        return $this->gd_render("barbecue_rentals/index", [
    ...
    public function view($id)
    {
        ...
        return $this->gd_render("barbecue_rentals/view", [
    ...
    {
        try {
            $this->access->require("gd_barbecue_rentals_manage");
            if (!in_array($initial_mode, ["single", "recurring"], true)) {
                $initial_mode = "single";
            }
            $resources = $this->bookings->bookableResources(Constants::BARBECUE_RESOURCE_TYPE);
            $edit_data = null;
            ...
            if ($id > 0) {
                ...
                if (!$rental || in_array((string) $rental->status, ["cancelled", "completed", "archived"], true)) {
                ...
                if ((string) $rental->rental_type === "single") {
                    ...
                    try {
                        $local_start = $this->time->utcToLocal((string) $booking->starts_at_utc);
                        $local_end = $this->time->utcToLocal((string) $booking->ends_at_utc);
                    } catch (\Throwable $e) {
            ...
            return $this->gd_view("barbecue_rentals/rental_modal", [
                "resources" => $resources,
    ...
    public function link_modal()
    {
        try {
            ...
            if (!$rental) { return show_404(); }
            return $this->gd_view("barbecue_rentals/link_modal", ["rental" => $rental, "link_kinds" => ["primary", "replacement"]]);
        } catch (\Throwable $e) { $this->gd_fail($e); }
    }

    public function customer_options()
    {
        try { $this->access->require("gd_barbecue_rentals_manage"); $rows = $this->bookings->customerOptions((string) $this->request->getPost("q")); return $this->response->setJSON(["results" => array_map(static fn($row) => ["id" => (int) $row["id"], "text" => $row["display_name"] . " (" . app_lang("gd_account_type_" . $row["account_type"]) . ")"], $rows)]); }
        catch (\Throwable $e) { $this->gd_fail($e); }
    }

    public function contact_options()
    {
        try { $this->access->require("gd_barbecue_rentals_manage"); $rows = $this->bookings->contactOptions((int) $this->request->getPost("customer_account_id"), (string) $this->request->getPost("q")); return $this->response->setJSON(["results" => array_map(static fn($row) => ["id" => (int) $row["id"], "text" => $row["full_name"]], $rows)]); }
        catch (\Throwable $e) { $this->gd_fail($e); }
    }
    ...
    /** Disponibilidade de todas as churrasqueiras para o intervalo escolhido no formulário. */
    ...
    /**
     * Endpoint do formulário simplificado. O valor digitado pelo operador é
     * preservado no servidor, evitando divergência entre tela e persistência.
     */
    public function save_rental()
    {
        try {
            ...
            $input = $this->normalizeRentalFormInput($input, $mode);
    ...
    public function single_data()
    {
        try {
            ...
            foreach ($result["data"] as $row) {
                $rows[] = $this->singleRow($row, $balances, $contacts);
            }
            $result["data"] = $rows;
            return $this->response->setJSON($result);
        } catch (\Throwable $e) { $this->gd_fail($e); }
    ...
    public function update_single()
    {
        ...
        try {
            ...
            foreach ($rental->links as $link) {
                if ((string) ($link->link_kind ?? "") !== "historical" && (int) ($link->booking_id ?? 0) > 0) {
                    $booking = $this->bookings->get((int) $link->booking_id);
                    break;
                }
            }
            if (!$booking) { throw new \DomainException("gd_court_rental_booking_not_found"); }
            $duration = $this->bookingDurationMinutes($booking);

            $input = $this->commercialInput() + $this->bookingInput();
            $input = $this->normalizeRentalFormInput($input, "single", $rental, $duration);
            if ($db->transBegin() === false) { throw new \RuntimeException("single identity update transaction"); }
            $in_tx = true;
            $input = $this->resolveMonthlyIdentity($input, $rental);
            $input["title"] = $this->rentalTitle($input, "single");
            $input["lock_version"] = $this->request->getPost("lock_version");
            $input["booking_lock_version"] = $this->request->getPost("booking_lock_version");
            $result = $this->service->updateSingle($id, $input);
            if ($db->transCommit() === false) { throw new \RuntimeException("single identity update commit"); }
            $in_tx = false;
            $this->json_success(app_lang("record_saved"), $result);
        } catch (\Throwable $e) {
    ...
    public function register_extra_time()
    {
        $this->emit_http_status = true;
        try {
            $this->access->require("gd_barbecue_rentals_manage");
            $result = $this->service->registerExtraTime((int) $this->request->getPost("rental_id"), [
                "extra_time_minutes" => $this->request->getPost("extra_time_minutes"),
                "extra_time_amount" => $this->request->getPost("extra_time_amount"),
                "extra_time_notes" => $this->request->getPost("extra_time_notes"),
                "lock_version" => $this->request->getPost("lock_version"),
            ]);
            $this->json_success(app_lang("gd_extra_time_saved"), $result);
        } catch (\Throwable $e) { $this->gd_fail($e); }
    }
    ...
    public function update_monthly()
    {
        ...
        try {
            ...
            $input = $this->commercialInput() + $this->seriesInput();
            $input = $this->normalizeRentalFormInput($input, "recurring", $rental, $this->seriesDurationMinutes($existing_series));
    ...
    public function reprice()
    {
        try {
            $this->access->require("gd_barbecue_rentals_manage");
            $result = $this->service->reprice((int) $this->request->getPost("id"), $this->commercialInput() + ["lock_version" => $this->request->getPost("lock_version")], $this->access->can("gd_barbecue_rentals_price_override"));
            $this->json_success(app_lang("record_saved"), $result);
        } catch (\Throwable $e) { $this->gd_fail($e); }
    }
    ...
    {
        $this->writeStatus(fn() => (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->activate((int) $id, (int) $this->request->getPost("lock_version"), $this->access->can("gd_barbecue_rentals_price_override"), (string) $this->request->getPost("justification")));
    }
    public function suspend($id)
    {
        $this->writeStatus(fn() => (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->suspend((int) $id, (int) $this->request->getPost("lock_version"), "keep", (string) $this->request->getPost("reason")));
    }
    public function resume($id)
    {
        $this->writeStatus(fn() => (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->resume((int) $id, (int) $this->request->getPost("lock_version")));
    }
    public function cancel($id)
    {
        $this->writeStatus(fn() => (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->cancel((int) $id, (int) $this->request->getPost("lock_version"), (string) $this->request->getPost("reason")));
    }
    public function complete($id)
    {
        $this->writeStatus(fn() => (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->complete((int) $id, (int) $this->request->getPost("lock_version")));
    }

    private function maybeActivate(array &$result, int $id, int $lock_version): void
    {
        if (!$this->request->getPost("activate")) { return; }
        $row = (new BarbecueRentalLifecycleService($this->unit_id, $this->user_id(), $this->login_user))->activate($id, $lock_version, $this->access->can("gd_barbecue_rentals_price_override"), (string) $this->request->getPost("justification"));
    ...
    private function writeStatus(callable $operation): void
    {
        ...
        $this->emit_http_status = true;
        try { $this->access->require("gd_barbecue_rentals_status_manage"); $row = $operation(); $this->json_success(app_lang("record_saved"), ["id" => (int) $row->id, "status" => (string) $row->status, "lock_version" => (int) $row->lock_version]); }
        catch (\Throwable $e) { $this->gd_fail($e); }
    }
    ...
    private function normalizeRentalFormInput(array $input, string $mode, ?object $existing_rental = null, int $existing_duration = 0): array
    {
        ...
        $resource_ok = db_connect()->table(db_connect()->prefixTable("gd_resources"))
            ->where("id", $resource_id)->where("unit_id", $this->unit_id)->where("resource_type", Constants::BARBECUE_RESOURCE_TYPE)
    ...
    private function rentalTitle(array $input, string $mode): string
    {
        ...
        $resource_code = trim((string) ($resource->code ?? "Churrasqueira"));
        if ($mode === "recurring") {
    ...
    private function bookingDurationMinutes(object $booking): int
    {
        try {
            $start = $this->time->parseUtc((string) $booking->starts_at_utc);
            $end = $this->time->parseUtc((string) $booking->ends_at_utc);
    ...
    private function resolveMonthlyIdentity(array $input, object $rental): array
    {
        ...
        if ($customer_name === "") { throw new \DomainException("gd_barbecue_customer_required"); }
    ...
    {
        $view_uri = get_uri("grupo_donato/barbecue-rentals/view/" . $row->id);
        $actions = anchor($view_uri, "<i data-feather='eye' class='icon-16'></i>", ["title" => app_lang("gd_view_details")]);
        if ($this->access->can("gd_barbecue_rentals_manage") && !in_array((string) $row->status, ["cancelled", "completed", "archived"], true)) {
            $modal = (string) $row->rental_type === "recurring" ? "monthly-modal" : "single-modal";
            $actions .= " " . modal_anchor(get_uri("grupo_donato/barbecue-rentals/" . $modal), "<i data-feather='edit' class='icon-16'></i>", ["title" => app_lang("edit"), "class" => "edit ms-2", "data-post-id" => (int) $row->id]);
        }
    ...
    private function singleRow(object $row, array $balances, array $contacts): array
    {
        ...
        $schedule = trim((string) ($row->schedule["display"] ?? ""));
        ...
        return [
            ...
            $this->rentalActions($row, $balances, "single-modal", "gd-barbecue-rentals-table", "avulso"),
        ];
    }
    ...
    private function monthlyFinance(object $row, array $balances): string
    {
        ...
        if (DataNormalizationService::decimalCompare($overdue, "0.00") > 0) {
            return '<span class="badge bg-danger">' . app_lang("gd_finance_overdue") . " " . $this->escape(to_currency($overdue)) . "</span>";
        }
        $status = (string) ($info["status"] ?? "unpaid");
        if ($status === "paid" || DataNormalizationService::decimalCompare($bal, "0.00") === 0) {
            return '<span class="badge bg-success">' . app_lang("gd_finance_paid") . "</span>";
        }
        $key = $status === "deposit_only" ? "gd_finance_deposit_only" : ($status === "partial" ? "gd_finance_partial" : "gd_finance_unpaid");
    ...
    private function rentalActions(object $row, array $balances, string $modal, string $reload_target, string $edit_label): string
    {
        ...
        $html = anchor(get_uri("grupo_donato/barbecue-rentals/view/" . $id), "<i data-feather='eye' class='icon-16'></i>", ["title" => app_lang("gd_view_details"), "class" => "me-2"]);
        if ($this->access->can("gd_barbecue_rentals_manage") && !in_array($status, ["cancelled", "completed", "archived"], true)) {
            $html .= modal_anchor(get_uri("grupo_donato/barbecue-rentals/" . $modal), "<i data-feather='edit' class='icon-16'></i>", ["title" => app_lang("edit") . " " . $edit_label, "class" => "edit me-2", "data-post-id" => $id]);
        }
        if ($this->access->can("gd_barbecue_rentals_status_manage")) {
        ...
        if ($this->access->can("gd_rental_payments_manage")) {
            ...
            if (count($open_ids) === 1) {
                ...
                $html .= '<div class="dropdown d-inline-block me-2">';
                $html .= '<button type="button" class="btn btn-link p-0" data-bs-toggle="dropdown" aria-expanded="false" title="' . app_lang("gd_finance_register_payment") . '"><i data-feather="dollar-sign" class="icon-16"></i></button>';
                $html .= '<ul class="dropdown-menu">';
                foreach (array_values($open_ids) as $index => $open_id) {
                    $html .= '<li>' . modal_anchor(get_uri("grupo_donato/finance/rental-payment-modal"), "Parcela " . ((int) $index + 1), ["class" => "dropdown-item", "title" => app_lang("gd_finance_register_payment"), "data-post-receivable_id" => (int) $open_id, "data-post-reload_target" => $reload_target, "data-modal-class" => "gd-payment-modal"]) . '</li>';
                }
                $html .= '</ul></div>';
            }
        }
    ...
    {
        // O horário local canônico agora vem do service (BarbecueRentalService::scheduleSummary),
