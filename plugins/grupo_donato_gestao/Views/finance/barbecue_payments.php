...
$turmas_pagamento = [["id" => "", "text" => "Todas as churrasqueiras"]];
foreach ($resources as $resource) {
    $turmas_pagamento[] = ["id" => (string) $resource["id"], "text" => $resource["code"] . " - " . $resource["name"]];
}
...
<style>
    ...
    @media (max-width: 767.98px) {
        ...
        #bombeiros-pagamentos-table td.option .btn {
            font-size: 12px;
...
<div class="p20">
    ...
    <div class="gd-mobile-filter-panel p15 mb15">
        <div class="row">
            ...
            <div class="col-sm-6">
                <label for="bombeiros-pagamentos-mobile-turma">Churrasqueira</label>
    ...
    window.initBombeirosPagamentosTable = function () {
        ...
        var dedupePagamentoRows = function () {
            ...
            table.rows().every(function (rowIndex) {
                ...
                var aluguelId = $actions.find("[data-rental-id]").first().attr("data-rental-id");
                var rowKey = cobrancaId ? "cobranca:" + cobrancaId : (aluguelId ? "mensalista:" + aluguelId + ":" + data[5] : data.join("|"));
        ...
        if (!$.fn.DataTable.isDataTable("#bombeiros-pagamentos-table")) {
            ...
            $("#bombeiros-pagamentos-table").appTable({
                source: "<?php echo_uri("grupo_donato/finance/barbecue-payments/data"); ?>",
                order: [[1, "asc"]],
                stateSave: false,
                ...
                columns: [
                    {title: "Churrasqueira", "class": "w90"},
                    {title: "Cliente", "class": "all w180"},
                    {title: "Contato", "class": "w180"},
                    {title: "WhatsApp", "class": "w130"},
                    {title: "Churrasqueira/Horário", "class": "w140"},
    ...
    $(document).ready(function () {
        ...
        var reloadPagamentos = function () {
            if (window.reloadBombeirosTable) {
                ...
                $("#bombeiros-pagamentos-table").appTable({reload: true});
            }
            if (window.reloadBombeirosPagamentosResumo) {
                reloadBombeirosPagamentosResumo();
            }
        ...
        window.reloadBombeirosPagamentosResumo = function () {
            appAjaxRequest({
                url: "<?php echo_uri("grupo_donato/finance/barbecue-payments/summary"); ?>",
                type: "POST",
                dataType: "json",
                data: {
                    ...
                    resource_id: getPagamentoFilterValue("resource_id", "")
                },
        ...
        var applyBombeirosPagamentosMobileFilters = function () {
            ...
            if (settings) {
                ...
                settings.filterParams.resource_id = $("#bombeiros-pagamentos-mobile-turma").val();
            }
        ...
        $("body").off("click", "#bombeiros-gerar-cobrancas-mes").on("click", "#bombeiros-gerar-cobrancas-mes", function () {
            ...
            appAjaxRequest({
                url: "<?php echo_uri("grupo_donato/finance/barbecue-payments/generate"); ?>",
                type: "POST",
                dataType: "json",
                data: {
                    mes_referencia: getPagamentoFilterValue("mes_referencia", "<?php echo (int) date("m"); ?>"),
        ...
        $("body").off("click", ".gd-rental-create-charge").on("click", ".gd-rental-create-charge", function () {
            var $link = $(this);
            $link.addClass("disabled");
            appAjaxRequest({
                url: "<?php echo_uri("grupo_donato/finance/barbecue-payments/create-rental-charge"); ?>",
                type: "POST",
                dataType: "json",
                data: {
                    rental_id: $link.data("rental-id"),
