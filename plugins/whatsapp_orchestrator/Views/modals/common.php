            ...
            <div class="modal-body">
                ...
                <div class="impulso-campaign-form-step impulso-hidden" data-campaign-panel="2">
                    <div class="impulso-grid impulso-grid-2">
                        <div class="impulso-field-grid">
                            <div class="impulso-field full"><label>Quem vai receber?</label><select class="form-control" id="impulso-campaign-audience-source"><option value="students">Pais dos alunos cadastrados</option><option value="contacts">Contatos do WhatsApp</option><option value="manual">Adicionar contatos</option><option value="csv">Enviar uma planilha</option></select></div>
                            <div class="impulso-field full" id="impulso-campaign-contact-filters">
                                <div class="impulso-field-grid">
                                    <div class="impulso-field"><label>Etiquetas incluídas</label><input class="form-control" id="impulso-campaign-include-tags" placeholder="lead, interessado"></div>
                                    <div class="impulso-field"><label>Etiquetas excluídas</label><input class="form-control" id="impulso-campaign-exclude-tags" placeholder="matriculado, opt-out"></div>
                                </div>
                            </div>
                            <div class="impulso-field full impulso-hidden" id="impulso-campaign-students-panel">
                                <div class="impulso-audience-source-callout"><i data-feather="book-open"></i><div><strong>Responsáveis dos alunos</strong><span>O sistema busca os alunos cadastrados e usa o WhatsApp do responsável. Telefones repetidos entram uma única vez.</span></div><select class="form-control" id="impulso-campaign-student-status"><option value="active">Somente alunos ativos</option><option value="all">Todos cadastrados</option></select></div>
                            </div>
                            <div class="impulso-field full impulso-hidden" id="impulso-campaign-recipient-editor">
                                <div class="impulso-recipient-toolbar"><div><strong>Destinatários</strong><span>Preencha uma pessoa por linha. Os campos de nome podem ser usados na mensagem.</span></div><button class="btn btn-default btn-sm" type="button" data-impulso-action="add-campaign-recipient"><i data-feather="plus"></i> Adicionar</button></div>
                                <div class="impulso-recipient-list" id="impulso-campaign-manual-recipient-list"></div>
                                <textarea class="impulso-hidden" id="impulso-campaign-manual-numbers" aria-hidden="true"></textarea>
                            </div>
                            <div class="impulso-field full impulso-hidden" id="impulso-campaign-spreadsheet-panel">
                                <div class="impulso-spreadsheet-callout"><div class="impulso-spreadsheet-copy"><i data-feather="file-text"></i><div><strong>Importe sua planilha</strong><span>Use o modelo, preencha os telefones e os nomes e envie o arquivo aqui.</span></div></div><div class="impulso-inline impulso-gap-8"><button class="btn btn-default btn-sm" type="button" data-impulso-action="download-campaign-audience-template"><i data-feather="download"></i> Baixar modelo</button><button class="btn btn-primary btn-sm" type="button" data-impulso-action="import-campaign-audience"><i data-feather="upload"></i> Escolher planilha</button></div></div>
                                <input type="file" id="impulso-campaign-audience-file" class="impulso-hidden" accept=".xlsx,.xls,.csv,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel">
                                <div class="impulso-import-status" id="impulso-campaign-import-status">Nenhuma planilha carregada.</div>
                            </div>
                        </div>
                ...
                <div class="impulso-campaign-form-step impulso-hidden" data-campaign-panel="3">
                    <div class="impulso-field-grid impulso-mb-14 impulso-hidden" id="impulso-campaign-official-fields">
                        ...
                        <div class="impulso-field full impulso-hidden"><label>Componentes do template</label><textarea class="form-control impulso-code-input" id="impulso-campaign-template-parameters" rows="6">[]</textarea></div>
                    </div>
                    <div class="impulso-grid impulso-grid-2">
                        <div>
                            <div class="impulso-field full"><label>Mensagem / prévia</label><textarea class="form-control" id="impulso-campaign-message" rows="10" placeholder="Olá, {nome}! Ainda tem interesse..."></textarea><div class="impulso-inline impulso-gap-8 impulso-mt-8" id="impulso-campaign-freeform-tools"><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-emoji"><i data-feather="smile"></i> Emoji</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{nome}">{nome}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{nome_aluno}">{nome_aluno}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{matricula}">{matricula}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{turma}">{turma}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{var1}">{var1}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-variable" data-variable="{var2}">{var2}</button><button class="btn btn-default btn-sm" type="button" data-impulso-action="campaign-attachment"><i data-feather="paperclip"></i> Mídia</button></div><input type="file" id="impulso-campaign-file" hidden accept="image/*,video/*,audio/*,.pdf"></div>
                        </div>
