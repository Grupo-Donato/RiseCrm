...
<div id="page-content" class="page-wrapper clearfix gd-rentals-shell">
    <div class="page-title clearfix gd-page-header">
        ...
        <div class="title-button-group gd-toolbar">
            <?php if (!empty($can_manage) && !in_array($status, ["cancelled", "completed", "archived"], true)) { echo modal_anchor(get_uri("grupo_donato/barbecue-rentals/" . $edit_modal), '<i data-feather="edit" class="icon-16"></i> ' . app_lang("edit"), ["class" => "btn btn-primary", "data-post-id" => (int) $rental->id, "title" => app_lang("edit")]); } ?>
            <?php if (!empty($can_calendar)) { echo anchor(get_uri("grupo_donato/barbecue-calendar"), '<i data-feather="calendar" class="icon-16"></i> ' . app_lang("gd_open_agenda"), ["class" => "btn btn-default"]); } ?>
            <?php echo anchor(get_uri("grupo_donato/barbecue-rentals"), '<i data-feather="arrow-left" class="icon-16"></i> ' . app_lang("back"), ["class" => "btn btn-default"]); ?>
        </div>
    </div>
            ...
            <div class="card mb15">
                <div class="card-body">
                    <div class="gd-detail-grid">
                        ...
                        <div class="gd-detail-item">
                            <small class="text-muted"><?php echo app_lang("gd_barbecues"); ?></small>
    ...
    <?php echo csrf_field(); ?>
            ...
            <?php if (!empty($can_generate_receivable) && ($rental->rental_type ?? "") === "single" && $primary_amount !== null && empty($financial["receivables"])) { ?>
                <div class="card mb15">
                    ...
                    <div class="card-body">
                        <?php echo form_open(get_uri("grupo_donato/finance/generate-barbecue-rental"), ["id" => "gd-cr-generate-receivable", "class" => "general-form"]); ?>
                            <input type="hidden" name="rental_id" value="<?php echo (int) $rental->id; ?>">
                            <div class="row">
                                <div class="col-md-4">
                ...
                <div class="page-title clearfix">
                    ...
                    <?php if ($can_manage && !in_array($status, ["cancelled", "completed", "archived"], true)) { ?>
                        <div class="title-button-group">
                            <?php echo modal_anchor(get_uri("grupo_donato/barbecue-rentals/link-modal"), '<i data-feather="link-2" class="icon-16"></i> ' . app_lang("gd_link_existing"), ["class" => "btn btn-default", "data-post-id" => $rental->id]); ?>
                        </div>
                    <?php } ?>
                </div>
...
$(document).ready(function(){
    ...
    $("#gd-cr-actions button").on("click", function(){
        ...
        postAction('<?php echo_uri("grupo_donato/barbecue-rentals/" . (int) $rental->id . "/"); ?>' + button.data("action"), data);
    });

    $("#gd-cr-reprice").on("submit", function(event){
        event.preventDefault();
        postAction('<?php echo_uri("grupo_donato/barbecue-rentals/reprice"); ?>', $(this).serialize());
    });

    $("#gd-cr-extra-time").on("submit", function(event){
        event.preventDefault();
        postAction('<?php echo_uri("grupo_donato/barbecue-rentals/extra-time"); ?>', $(this).serialize());
    });
