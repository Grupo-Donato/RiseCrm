...
{
    public function findConflicts(int $unit_id,array $resource_windows,int|array|null $exclude_booking_id=null):array
    {
        ...
        $excluded=is_array($exclude_booking_id)?array_values(array_filter(array_map("intval",$exclude_booking_id))):[(int)$exclude_booking_id];$excluded=array_values(array_filter($excluded,static fn(int $id):bool=>$id>0));if(count($excluded)===1){$q->where("$b.id !=",$excluded[0]);}elseif($excluded){$q->whereNotIn("$b.id",$excluded);}$q->groupStart();foreach($windows as $i=>$w){$method=$i===0?"groupStart":"orGroupStart";$q->$method()->where("$br.resource_id",$w["resource_id"])->where("$br.occupancy_starts_at_utc <",$w["end"])->where("$br.occupancy_ends_at_utc >",$w["start"])->groupEnd();}$q->groupEnd();
