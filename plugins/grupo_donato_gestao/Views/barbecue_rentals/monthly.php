...
if (!empty($can_manage)) {
    $buttons[] = modal_anchor(get_uri("grupo_donato/barbecue-rentals/monthly-modal"), '<i data-feather="plus-circle" class="icon-16"></i> ' . app_lang("gd_new_barbecue_rental_monthly"), ["class" => "btn btn-primary", "title" => app_lang("gd_new_barbecue_rental_monthly")]);
}
?>
...
<div id="page-content" class="page-wrapper clearfix gd-rentals-shell">
    <div class="card">
        <div class="page-title clearfix gd-page-header">
            <div>
                <h4><?php echo app_lang("gd_barbecue_monthly_renters"); ?></h4>
            </div>
...
$(document).ready(function(){
    $("#gd-court-renters-table").appTable({
        source:'<?php echo_uri("grupo_donato/barbecue-rentals/monthly-data"); ?>',
        serverSide:true,
        order:[[0,"asc"]],
        filterDropdown:[
