<style>
    ...
    .impulso-more-menu button:hover,
    .impulso-more-menu button:focus-visible { background: var(--ih-surface-soft); }
    .impulso-more-menu svg { height: 14px; width: 14px; }
    ...
    .impulso-contact-drawer-header {
        align-items: center;
        background: var(--ih-surface-raised, var(--ih-surface-soft));
        border-bottom: 1px solid var(--ih-border);
        display: flex;
        justify-content: space-between;
        padding: 9px 12px;
    ...
    .impulso-contact-drawer-header > div {
        display: flex;
        flex-direction: column;
        gap: 2px;
    }

    .impulso-contact-drawer-header strong { font-size: 12px; }

    .impulso-contact-tabs {
        border-bottom: 1px solid var(--ih-border);
        display: flex;
        gap: 2px;
        padding: 6px 8px 0;
    ...
    .impulso-contact-tabs button:hover,
    .impulso-contact-tabs button.active {
        border-bottom-color: var(--ih-primary);
        color: var(--ih-primary);
    }

    .impulso-contact-tab-panel[hidden] { display: none; }
    ...
    /* Multi-instance channels are presented by the compact picker in the list header. */
    ...
    .impulso-current-channel span {
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
    }

    .impulso-current-channel svg {
        flex: 0 0 auto;
        height: 12px;
        width: 12px;
    }

    .impulso-channel-picker {
        background: var(--ih-surface, #fff);
        border: 1px solid var(--ih-border);
        border-radius: 10px;
        box-shadow: var(--ih-shadow, 0 12px 30px rgba(31, 41, 55, .16));
        left: 12px;
    ...
    .impulso-channel-picker-heading {
        border-bottom: 1px solid var(--ih-border);
        display: flex;
        flex-direction: column;
        gap: 2px;
    ...
    .impulso-channel-picker-heading strong { font-size: 11px; }
    .impulso-channel-picker-heading span { font-size: 9px; opacity: .68; }
    .impulso-channel-picker .impulso-channel-list { padding: 7px; }
    .impulso-channel-picker .impulso-channel-item { margin-bottom: 2px; }
    .impulso-channel-picker .impulso-channel-copy small { opacity: .72; }
    .impulso-channel-picker .impulso-channel-manage { min-height: 40px; }

    .impulso-status-filter {
        align-items: center;
        display: inline-flex;
        flex: 0 0 auto;
        gap: 4px;
    ...
    .impulso-status-filter span {
        font-size: 8px;
        opacity: .66;
        text-transform: uppercase;
    }

    .impulso-status-filter select {
        background: transparent;
        border: 1px solid var(--ih-border);
        border-radius: 6px;
        color: inherit;
    ...
    @container impulso-inbox (max-width: 1480px) {
        .impulso-chat-layout { grid-template-columns: minmax(330px, 360px) minmax(0, 1fr); }
        .impulso-contact-sidebar {
            bottom: 0;
            max-width: 330px;
            position: fixed;
    ...
    @container impulso-inbox (max-width: 1100px) {
        .impulso-chat-layout { grid-template-columns: minmax(310px, 340px) minmax(0, 1fr); }
    }
    ...
    .impulso-hub .impulso-message-dialog-backdrop,
    .impulso-hub .impulso-inbox-dialog-backdrop { align-items: center; background: rgba(0,0,0,.34); display: flex; inset: 0; justify-content: center; padding: 16px; position: fixed; z-index: 1090; }
    .impulso-hub .impulso-message-dialog,
    .impulso-hub .impulso-inbox-dialog { background: var(--ih-surface); border: 1px solid var(--ih-border); border-radius: 12px; box-shadow: 0 18px 48px rgba(0,0,0,.24); max-width: 460px; padding: 16px; width: 100%; }
    ...
    .impulso-hub .impulso-message-dialog-form label,
    .impulso-hub .impulso-inbox-dialog-form label { display: block; font-size: 10px; font-weight: 700; margin-bottom: 9px; }
    .impulso-hub .impulso-message-dialog-form input,
    .impulso-hub .impulso-message-dialog-form textarea,
    .impulso-hub .impulso-inbox-dialog-form input { background: transparent; border: 1px solid var(--ih-border); border-radius: 7px; color: inherit; display: block; margin-top: 4px; padding: 8px; width: 100%; }
    ...
    /* Inbox 4 foundation: one conversation list, one message surface, and overlay drawers. */
    .impulso-sr-only {
        border: 0;
        clip: rect(0, 0, 0, 0);
        clip-path: inset(50%);
    ...
    .impulso-hub .impulso-chat-layout {
        --impulso-conversation-track: minmax(340px, 380px);
        grid-template-columns: var(--impulso-conversation-track) minmax(0, 1fr);
    ...
    /*
     * Impulso Hub visual system
     *
     * The inbox is intentionally self-contained. Rise still owns the page
     * shell, but the workspace must keep a stable visual language across
     * themes instead of falling back to transparent/inherited surfaces.
     */
    .impulso-hub {
        --ih-page-bg: #061b32;
        --ih-surface: #0a2748;
        --ih-surface-raised: #0e345c;
        --ih-surface-soft: rgba(76, 132, 190, .14);
        --ih-control-bg: #103b65;
        --ih-border: rgba(142, 181, 221, .19);
        --ih-text: #f3f7fc;
        --ih-muted: #91adcb;
        --ih-primary: #7357ff;
        --ih-primary-strong: #8a73ff;
        --ih-primary-soft: rgba(115, 87, 255, .18);
        --ih-success: #13bd8a;
        --ih-warning: #f0ba35;
        --ih-danger: #ef6675;
        --ih-info: #69b7ff;
        --ih-shadow: 0 18px 48px rgba(1, 11, 25, .24);
        background: var(--ih-page-bg);
        border-radius: 8px;
        color: var(--ih-text);
    ...
    #page-content.impulso-page-content--conversations {
        background: var(--ih-page-bg, #061b32);
    }

    .impulso-hub .impulso-shell-card {
        background: var(--ih-page-bg);
        border: 1px solid rgba(142, 181, 221, .16);
        border-radius: 8px;
        box-shadow: var(--ih-shadow);
    }

    .impulso-hub .impulso-topbar {
        background: #0a2d53;
        border-bottom: 1px solid rgba(142, 181, 221, .16);
        min-height: 0;
        padding: 8px 20px;
    }

    .impulso-hub .impulso-topbar.impulso-topbar--compact {
        justify-content: flex-end;
        min-height: 0;
        padding: 8px 20px;
    }

    .impulso-hub .impulso-brand-block {
        gap: 14px;
    }

    .impulso-hub .impulso-brand-mark {
        background: linear-gradient(145deg, #765bff, #a287ff);
        border: 1px solid rgba(255, 255, 255, .12);
        border-radius: 14px;
        box-shadow: 0 10px 22px rgba(77, 52, 207, .34);
        height: 46px;
        width: 46px;
    }

    .impulso-hub .impulso-brand-block h1 {
        color: #fff;
        font-size: 22px;
        letter-spacing: -.02em;
    }

    .impulso-hub .impulso-eyebrow {
        color: #fff;
        font-size: 10px;
        letter-spacing: .14em;
        opacity: .96;
    ...
    .impulso-hub .impulso-live-pill {
        background: rgba(19, 189, 138, .12);
        border-color: rgba(19, 189, 138, .24);
        color: #3ed7a7;
        margin-left: 10px;
        padding: 6px 10px;
    }

    .impulso-hub .impulso-topbar-actions {
        gap: 10px;
    }

    .impulso-hub .impulso-topbar-actions .btn,
    .impulso-hub .impulso-topbar-actions .impulso-icon-button {
        border-radius: 8px;
        min-height: 40px;
    }

    .impulso-hub .impulso-command-button,
    .impulso-hub .impulso-notification-button,
    .impulso-hub .impulso-topbar-actions .btn-default {
        background: #174676;
        border-color: rgba(142, 181, 221, .16);
        color: #fff;
    }

    .impulso-hub .impulso-command-button:hover,
    .impulso-hub .impulso-notification-button:hover,
    .impulso-hub .impulso-topbar-actions .btn-default:hover {
        background: #1d5488;
    ...
    .impulso-hub .impulso-command-button kbd {
        background: rgba(4, 21, 40, .28);
        border-color: rgba(255, 255, 255, .13);
        color: #b9cee5;
    }

    .impulso-hub .btn-primary,
    .impulso-hub .impulso-send-button {
        background: var(--ih-primary);
        border-color: var(--ih-primary);
        color: #fff;
    }

    .impulso-hub .btn-primary:hover,
    .impulso-hub .impulso-send-button:hover {
        background: var(--ih-primary-strong);
        border-color: var(--ih-primary-strong);
        color: #fff;
    }

    .impulso-hub .btn-success {
        background: var(--ih-success);
        border-color: var(--ih-success);
        color: #fff;
    }

    .impulso-hub .btn-success:hover {
        background: #24d09d;
        border-color: #24d09d;
        color: #fff;
    }
    ...
    .impulso-hub .impulso-card {
        background: var(--ih-surface);
        border-color: var(--ih-border);
        box-shadow: 0 8px 24px rgba(1, 11, 25, .12);
    }

    .impulso-hub .impulso-card-header,
    .impulso-hub .impulso-table th,
    .impulso-hub .impulso-table td {
        border-color: var(--ih-border);
    }

    .impulso-hub .impulso-card-header p,
    .impulso-hub .impulso-section-heading p,
    .impulso-hub .impulso-stat-label,
    .impulso-hub .impulso-stat-trend,
    .impulso-hub .impulso-list-copy span,
    .impulso-hub .impulso-person-copy span {
        color: var(--ih-muted);
    }

    .impulso-hub .impulso-channel-picker {
        background: #0a2d52;
        border-color: var(--ih-border);
        box-shadow: 0 18px 42px rgba(1, 11, 25, .38);
    }

    .impulso-hub .impulso-channel-picker-heading {
        border-bottom-color: var(--ih-border);
    }

    .impulso-hub .impulso-channel-picker-heading strong { color: #fff; }
    .impulso-hub .impulso-channel-picker-heading span { color: var(--ih-muted); }

    .impulso-hub .impulso-contact-drawer-header {
        background: #0d355f;
        border-bottom-color: var(--ih-border);
    }

    .impulso-hub .impulso-contact-drawer-header strong { color: #fff; }

    .impulso-hub .impulso-contact-tabs {
        background: #0a2d52;
        border-bottom-color: var(--ih-border);
    }

    .impulso-hub .impulso-contact-tabs button { color: #a9c0db; }
    .impulso-hub .impulso-contact-tabs button:hover,
    .impulso-hub .impulso-contact-tabs button.active { color: #b5a8ff; }

    .impulso-hub .impulso-stat-icon,
    .impulso-hub .impulso-empty-icon {
        background: var(--ih-primary-soft);
        color: #a998ff;
    }

    .impulso-hub .impulso-count-badge,
    .impulso-hub .impulso-badge.primary {
        background: var(--ih-primary-soft);
        color: #ad9fff;
    }

    .impulso-hub .form-control,
    .impulso-hub input.form-control,
    .impulso-hub select.form-control,
    .impulso-hub textarea.form-control {
        background: var(--ih-control-bg);
        border-color: rgba(142, 181, 221, .18);
        color: var(--ih-text);
    }

    .impulso-hub .form-control:focus {
        background: #144674;
        border-color: rgba(115, 87, 255, .78);
        box-shadow: 0 0 0 3px rgba(115, 87, 255, .18);
        color: #fff;
    }

    .impulso-hub .form-control::placeholder,
    .impulso-hub input::placeholder,
    .impulso-hub textarea::placeholder {
        color: #8eaccb;
        opacity: 1;
    }
    ...
    .impulso-hub .impulso-chat-sidebar {
        background: #0a2d52;
        border-right-color: var(--ih-border);
    }

    .impulso-hub .impulso-chat-sidebar-header {
        background: #0d355f;
        border-bottom-color: var(--ih-border);
    ...
    .impulso-hub .impulso-search input {
        background: #123d68;
        border-color: rgba(142, 181, 221, .12);
        color: #fff;
        height: 40px;
    }

    .impulso-hub .impulso-search input:focus {
        background: #164774;
        border-color: rgba(115, 87, 255, .74);
    }

    .impulso-hub .impulso-workflow-filters {
        gap: 6px;
        margin-top: 9px;
    }

    .impulso-hub .impulso-workflow-filters .form-control {
        background: #123d68;
        border-color: rgba(142, 181, 221, .12);
        color: #dbe8f7;
        font-size: 10px;
    ...
    .impulso-hub .impulso-queue-tabs {
        background: #0a2d52;
        border-bottom-color: var(--ih-border);
        gap: 2px;
        flex-wrap: wrap;
    ...
    .impulso-hub .impulso-queue-tabs::-webkit-scrollbar {
        display: none;
    }
    ...
    .impulso-hub .impulso-queue-tab:hover {
        color: #fff;
    }

    .impulso-hub .impulso-queue-tab.active {
        background: var(--ih-primary-soft);
        color: #b5a8ff;
    }

    .impulso-hub .impulso-status-filter select {
        background: #123d68;
        border-color: rgba(142, 181, 221, .18);
        color: #dbe8f7;
    }

    .impulso-hub .impulso-conversation-list {
        background: #0a2d52;
    }

    .impulso-hub .impulso-conversation-item {
        background: transparent;
        border-bottom-color: rgba(142, 181, 221, .13);
        padding: 13px 12px;
    }

    .impulso-hub .impulso-conversation-item:hover {
        background: rgba(83, 143, 205, .12);
    }

    .impulso-hub .impulso-conversation-item.active {
        background: #173f75;
        box-shadow: inset 3px 0 0 var(--ih-primary);
    }

    .impulso-hub .impulso-conversation-item.unread .impulso-conversation-title strong {
        color: #fff;
    }

    .impulso-hub .impulso-avatar {
        background: linear-gradient(145deg, #3742ad, #6955e6);
        border-color: rgba(175, 164, 255, .28);
        color: #fff;
    }

    .impulso-hub .impulso-workflow-pill,
    .impulso-hub .impulso-workflow-tag {
        background: rgba(80, 139, 198, .18);
        border-color: rgba(142, 181, 221, .13);
        color: #bcd2e9;
    }

    .impulso-hub .impulso-unread {
        background: var(--ih-primary);
    }
    ...
    .impulso-hub .impulso-chat-header-copy h3 {
        color: #fff;
        font-size: 15px;
    }
    ...
    .impulso-hub .impulso-active-channel-chip {
        background: rgba(19, 189, 138, .12);
        color: #3ed7a7;
    }

    .impulso-hub .impulso-chat-header-actions .btn-default,
    .impulso-hub .impulso-chat-header-actions .impulso-icon-button {
        background: #174676;
        border-color: rgba(142, 181, 221, .17);
        color: #e8f1fb;
    }

    .impulso-hub .impulso-chat-header-actions .btn-default:hover,
    .impulso-hub .impulso-chat-header-actions .impulso-icon-button:hover,
    .impulso-hub .impulso-chat-header-actions .btn-default.active {
        background: #235789;
        border-color: rgba(142, 181, 221, .36);
        color: #fff;
    }

    .impulso-hub .impulso-chat-body {
        background-color: #082848;
        background-image: radial-gradient(rgba(116, 169, 222, .12) 1px, transparent 1px);
        background-size: 19px 19px;
        padding: 22px 28px;
    }

    .impulso-hub .impulso-day-divider {
        color: #98b4d1;
    }

    .impulso-hub .impulso-day-divider::before,
    .impulso-hub .impulso-day-divider::after {
        background: rgba(142, 181, 221, .2);
    }

    .impulso-hub .impulso-message {
        background: #12395f;
        border-color: rgba(142, 181, 221, .17);
        box-shadow: 0 5px 18px rgba(1, 11, 25, .15);
        color: #e9f2fb;
        padding: 10px 12px 8px;
    }

    .impulso-hub .impulso-message-row.outgoing .impulso-message {
        background: #7357ff;
        border-color: #7357ff;
        color: #fff;
    }

    .impulso-hub .impulso-message-row.note .impulso-message {
        background: rgba(240, 186, 53, .14);
        border-color: rgba(240, 186, 53, .32);
        color: #ffe5a0;
    }
    ...
    .impulso-hub .impulso-message-row.outgoing .impulso-message-footer {
        color: rgba(255, 255, 255, .76);
    }

    .impulso-hub .impulso-composer {
        background: #082744;
        border-top-color: var(--ih-border);
        padding: 12px 16px 14px;
    }

    .impulso-hub .impulso-composer-mode {
        gap: 7px;
    }
    ...
    .impulso-hub .impulso-mode-button.active {
        background: var(--ih-primary-soft);
        color: #b3a6ff;
    }
    ...
    .impulso-hub .impulso-composer-box {
        background: #123d68;
        border-color: rgba(142, 181, 221, .2);
        border-radius: 12px;
        padding: 8px;
    }

    .impulso-hub .impulso-composer-box:focus-within {
        border-color: rgba(115, 87, 255, .74);
        box-shadow: 0 0 0 3px rgba(115, 87, 255, .16);
    }
    ...
    .impulso-hub .impulso-tool-button:hover {
    ...
    .impulso-hub .impulso-contact-sidebar {
        background: #0a2d52;
        border-left-color: var(--ih-border);
        bottom: 0;
        box-shadow: -18px 0 40px rgba(1, 11, 25, .32);
        max-width: 360px;
        position: absolute;
        right: -380px;
        top: 0;
    ...
    .impulso-hub .impulso-contact-sidebar.open {
        right: 0;
    }

    .impulso-hub .impulso-contact-profile,
    .impulso-hub .impulso-contact-section {
        border-color: var(--ih-border);
    }
    ...
    .impulso-hub .impulso-contact-profile h3,
    .impulso-hub .impulso-contact-section-title,
    .impulso-hub .impulso-contact-item-copy strong {
        color: #fff;
    }

    .impulso-hub .impulso-contact-profile p,
    .impulso-hub .impulso-contact-item-copy span,
    .impulso-hub .impulso-contact-section small,
    .impulso-hub .impulso-contact-section > small {
        color: var(--ih-muted);
    }
    ...
    .impulso-hub .impulso-profile-action:hover {
        background: #1a4e80;
        border-color: rgba(115, 87, 255, .6);
        color: #fff;
    }

    .impulso-hub .impulso-contact-item > svg {
        color: #8eaece;
    }
    ...
    .impulso-hub .impulso-hidden {
        display: none !important;
    }

    .impulso-hub :where(button, a, input, select, textarea):focus-visible {
        outline-color: #a998ff;
    }

    @media (max-width: 1280px) {
        .impulso-hub .impulso-chat-layout {
            --impulso-conversation-track: minmax(320px, 360px);
        }
    }
    ...
    @media (max-width: 991.98px) {
        ...
        .impulso-hub .impulso-contact-sidebar.open { right: 0; }
    }
