...
class Finance extends Gd_Controller
{
    ...
    public function reverse_payment(){try{$this->access->require('gd_payments_manage');$this->service->reversePayment((int)$this->request->getPost('id'),(string)$this->request->getPost('reason'));$this->json_success(app_lang('gd_rental_payments_undo_success'));}catch(\Throwable $e){$this->gd_fail($e);}}
    public function receipt($id){$p=$this->service->getPayment((int)$id);if(!$p)return show_404();return $this->gd_render('finance/receipt',['payment'=>$p]);}
